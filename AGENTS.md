...
- After completing a change, if `bun run lint` and `bun run test` pass, commit and push by default unless the user says not to.
- Do not run `bun run build` or any production build command unless the user explicitly asks for it in that thread.
